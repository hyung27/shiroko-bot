...
export const createCanvasIntro = async (jid, sock, gcJid, ucapan, susu) => {
  return new Promise(async (resolve) => {
    ...
    try {
      const tt = await sock.groupMetadata(gcJid)
      title = tt.subject
      desc = tt.desc
      member = tt.participants.length
    } catch (error) {
      title = pushName
      desc = ""
      member = 0
    ...
    ctx.fillText(ucapan, posX, (posY * 2) + 120);
